...
import { speakWithVoice, speechText, useNovaVoice } from "@/lib/nova/nova-voice";
...
export const NovaConversationPanel = forwardRef<NovaConversationHandle, NovaConversationPanelProps>(
  function NovaConversationPanel(
    { onStatusChange, onListeningChange, onMicSupportedChange, className },
    ref,
  ) {
  ...
  el.scrollTop = el.scrollHeight;
    ...
    async function send(raw: string) {
      ...
      try {
        const full = await streamNovaReply({
          ...
          onChunk: (fullText) => {
            if (!started) {
              ...
              setMessages((prev) => [
                ...prev,
                { id: replyId, role: "assistant", text: fullText, at: Date.now() },
              ]);
        ...
        if (!muted && full) {
          report("speaking", "happy");
          speakWithVoice(speechText(full), novaVoice, {
            onBoundary: () => report("speaking", "happy"),
            onEnd: () => report("idle", expression),
          });
        } else {
