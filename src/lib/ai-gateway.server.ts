...
const AI_RUN_ID_HEADER = "X-AI-Run-ID";
...
export function createAiGatewayRunIdFetch(initialRunId?: string) {
  ...
  return {
    fetch: async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (runId && !headers.has(AI_RUN_ID_HEADER)) {
        headers.set(AI_RUN_ID_HEADER, runId);
      }
      try {
        const response = await fetch(input, { ...init, headers });
        publishRunId(response.headers.get(AI_RUN_ID_HEADER) ?? undefined);
        return response;
      } catch (error) {
...
export function getAiGatewayRunId(request: Request) {
  return request.headers.get(AI_RUN_ID_HEADER)?.trim() || undefined;
}
...
export function getAiGatewayResponseHeaders(
  ...
  new Headers(providerHeaders).forEach((value, name) => {
    if (name.toLowerCase().startsWith("x-ai-")) {
  ...
  headers.forEach((_, name) => {
    if (name.toLowerCase().startsWith("x-ai-")) exposedHeaders.add(name);
  });
...
export async function withAiGatewayRunIdHeader(
  response: Response,
  gateway: {
  ...
  if (!response.body) {
    ...
    const headers = getAiGatewayResponseHeaders(undefined, response.headers);
    new Headers(init).forEach((value, name) => headers.set(name, value));
    if (runId) headers.set(AI_RUN_ID_HEADER, runId);
    return new Response(response.body, {
      status: response.status,
      statusText: response.statusText,
      headers: getAiGatewayResponseHeaders(undefined, headers),
    });
  }
