...
function deriveWarnings(s: VisionStatus): EnvironmentWarning[] {
  ...
  if (s.people > 1)
    out.push({ id: "people", message: "Multiple person detected in frame", level: "bad" });
  if (s.phone)
    out.push({ id: "phone", message: "Please don't use your phone during the interview", level: "bad" });
