...
type ChatRequestBody = { messages?: unknown; interviewerMode?: unknown; plain?: unknown };
...
function normalizeMessages(raw: unknown): UIMessage[] {
  ...
  return raw
    .map((message) => {
      ...
      const text = (directText ?? directContent ?? parts.map((part) => part.text).join(""))?.trim();
      if (!text) return null;
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        if (!key) return new Response("Missing OPENAI_API_KEY or GROQ_API_KEY", { status: 500 });

        const baseURL =
          process.env["OPENAI_BASE_URL"]?.trim() ||
          process.env["GROQ_BASE_URL"]?.trim() ||
          "https://api.groq.com/openai/v1";
        const modelId = process.env["AI_MODEL"]?.trim() || process.env["GROQ_MODEL"]?.trim() || "openai/gpt-oss-120b";
        ...
        const correctedMessages = await Promise.all(
          normalizedMessages.map(async (message) => {
            ...
            const nextText = await correctGrammarForChat(message.parts[0]?.text ?? "", gateway, modelId);
            return {
              ...message,
