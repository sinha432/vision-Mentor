...
import { analyzeResumeSentences, type SentenceInsight } from "@/interviewer/lib/resume-sentences";
  ...
  async function generateCorrections() {
    ...
    try {
      ...
      if ((result.corrections ?? []).length === 0) {
        toast.info("No further line changes found — the rewrite above still targets a 100/100 ATS score.");
      }
    } catch (error) {
        ...
        {!aiReady && (
          <div className="mt-6 rounded-2xl border border-warning/40 bg-warning/10 p-4 text-sm">
            ...
            <p className="mt-1 text-muted-foreground">
              Add <code className="font-mono">GROQ_API_KEY</code> to your <code className="font-mono">.env</code> file and
              restart the dev server. Resume ATS scoring works without it; live interviews and
              reports need it.
            </p>
          </div>
        )}
