...
import { Sparkles, Mail, Lock, Loader2, User, Calendar, Globe, KeyRound, Eye, EyeOff, Building2, UserCircle } from "lucide-react";
import { toast } from "sonner";
import { Toaster } from "@/components/ui/sonner";
import { Button } from "@/components/ui/button";
import {
  useDemoAuth, DEMO_EMAIL, DEMO_PASSWORD, DEMO_COMPANY_EMAIL, DEMO_COMPANY_PASSWORD,
  type UserRole,
...
function AuthPage() {
  ...
  useEffect(() => {
    if (ready && user) navigate({ to: redirectTo });
  }, [ready, user, navigate, redirectTo]);

  const fillDemo = (which: "individual" | "company") => {
    if (which === "company") { setEmail(DEMO_COMPANY_EMAIL); setPassword(DEMO_COMPANY_PASSWORD); }
    else { setEmail(DEMO_EMAIL); setPassword(DEMO_PASSWORD); }
  };
  ...
  const onSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate({ to: redirectTo });
    } catch (err: any) { toast.error(err?.message ?? "Could not create account"); }
    finally { setBusy(false); }
  };
      ...
      >
        ...
        {mode === "login" ? (
          <>
            <div className="glass rounded-lg p-3 mb-4 text-xs">
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1">
                  <div className="font-display uppercase tracking-widest text-[10px] text-muted-foreground">Demo accounts</div>
                  <div className="font-mono mt-1 text-[11px] break-all">{DEMO_EMAIL} <span className="text-muted-foreground">(individual)</span></div>
                  <div className="font-mono text-[11px] break-all">{DEMO_COMPANY_EMAIL} <span className="text-muted-foreground">(company)</span></div>
                  <div className="font-mono text-[11px] text-muted-foreground mt-0.5">password: {DEMO_PASSWORD}</div>
                </div>
              </div>
              <div className="flex gap-2 mt-2">
                <button onClick={() => fillDemo("individual")} className="flex-1 text-primary text-[11px] hover:underline">Use individual</button>
                <button onClick={() => fillDemo("company")} className="flex-1 text-primary text-[11px] hover:underline">Use company</button>
              </div>
            </div>
            ...
            <form onSubmit={onSignIn} className="space-y-3">
              ...
              <Field icon={<Lock className="w-4 h-4 text-muted-foreground" />}>
                <input type={showPw ? "text" : "password"} required value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="flex-1 bg-transparent outline-none py-3 text-sm" />
          ...
          <form onSubmit={onSignUp} className="space-y-3">
            <div>
              <div className="text-[10px] uppercase tracking-widest text-muted-foreground mb-1">Account type</div>
              <div className="grid grid-cols-2 gap-2">
                {([
                  { v: "individual" as const, label: "Individual", icon: <UserCircle className="w-4 h-4" /> },
                  { v: "company" as const, label: "Company", icon: <Building2 className="w-4 h-4" /> },
                ]).map((opt) => (
                  <button
                    key={opt.v}
                    type="button"
                    ...
                    className={`glass rounded-lg py-2.5 px-3 text-xs flex items-center justify-center gap-2 transition-all ${
                      role === opt.v ? "border border-primary text-foreground glow-primary" : "text-muted-foreground hover:text-foreground"
                    }`}
                  >
                    {opt.icon}<span className="uppercase tracking-widest font-display">{opt.label}</span>
                  </button>
                ))}
              </div>
            </div>
            <Field icon={<User className="w-4 h-4 text-muted-foreground" />}>
              <input required minLength={2} maxLength={60} value={name} onChange={(e) => setName(e.target.value)} placeholder={role === "company" ? "Company name" : "Full name"} className="flex-1 bg-transparent outline-none py-3 text-sm" />
            </Field>
            <Field icon={<Mail className="w-4 h-4 text-muted-foreground" />}>
            ...
            <Field icon={<Lock className="w-4 h-4 text-muted-foreground" />}>
              <input type={showSuPw ? "text" : "password"} required minLength={8} value={suPassword} onChange={(e) => setSuPassword(e.target.value)} placeholder="Password (8+ chars, letter & number)" className="flex-1 bg-transparent outline-none py-3 text-sm" />
              <PwToggle shown={showSuPw} onClick={() => setShowSuPw((v) => !v)} />
            </Field>
            <Field icon={<Lock className="w-4 h-4 text-muted-foreground" />}>
              <input type={showSuConfirm ? "text" : "password"} required minLength={8} value={suConfirm} onChange={(e) => setSuConfirm(e.target.value)} placeholder="Confirm password" className="flex-1 bg-transparent outline-none py-3 text-sm" />
