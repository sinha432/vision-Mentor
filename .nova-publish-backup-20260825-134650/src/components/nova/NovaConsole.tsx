...
export function NovaConsole({
   ...
   */
  const publishGeneratedAssessment = useCallback(async () => {
  if (publishing) return;

  if (!companyUserId?.trim()) {
    toast.error("Company account could not be identified.");
    return;
  }
  ...
  try {
    ...
    if (!questions.length) {
      throw new Error("No valid questions are available to publish.");
    }
    ...
    /*
     * Refresh the company assessment list.
     */
    ...
    /*
     * Clear Nova's temporary publishing state.
     */
    ...
    /*
     * Automatically return to the company's
     * Assessments section.
     */
    await navigate({
  ...
  const handleCompanyCommand =
    useCallback(
      (command: CompanyNovaCommand) => {
         ...
         * actual publishing
         */
        if (command.type === "publish_assessment") {
  if (!generatedQuestions.length) {
    toast.error(
      "No generated questions are ready. Generate questions first.",
    );
  ...
  toast.info(
    `I have ${generatedQuestions.length} generated ${
      generatedQuestions.length === 1
        ? "question"
        : "questions"
    } ready. Say "yes, publish it" to publish the assessment.`,
  );
